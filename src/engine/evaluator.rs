...
/// is_depthがtrueの場合に深さ優先探索を、falseの場合に幅湯煎探索をおkの合う
///
/// 実行時にエラーが起きた場合はErrを返す
/// マッチ成功時にはOk(true)を、失敗時はOk(false)を返す
...
pub fn eval_width(_inst: &[Instruction], _line: &[char]) -> Result<bool, EvalError> {
    Ok(false)
}
